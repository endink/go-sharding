...
type TxSerializer struct {
	*sync2.ConsolidatorCache
	...
	waits, waitsDryRun, queueExceeded, queueExceededDryRun, globalQueueExceeded, globalQueueExceededDryRun metric.Int64Counter
...
func New(config config.HotRowProtectionConfig) *TxSerializer {
	return &TxSerializer{
		...
		globalQueueExceededDryRun: telemetry.ExecutionMeter.NewInt64Counter(
			"TxSerializerGlobalQueueExceededDryRun",
			"Dry-run stats for TxSerializerGlobalQueueExceeded"),
		log:                          logutil.NewThrottledLogger("HotRowProtection", logutil.DefaultLogger, 5*time.Second),
		logDryRun:                    logutil.NewThrottledLogger("HotRowProtection DryRun", logutil.DefaultLogger, 5*time.Second),
		logWaitsDryRun:               logutil.NewThrottledLogger("HotRowProtection Waits DryRun", logutil.DefaultLogger, 5*time.Second),
		logQueueExceededDryRun:       logutil.NewThrottledLogger("HotRowProtection QueueExceeded DryRun", logutil.DefaultLogger, 5*time.Second),
		logGlobalQueueExceededDryRun: logutil.NewThrottledLogger("HotRowProtection GlobalQueueExceeded DryRun", logutil.DefaultLogger, 5*time.Second),
...
func (txs *TxSerializer) lockLocked(ctx context.Context, key, table string) (bool, error) {
	...
	if txs.globalSize >= txs.maxGlobalQueueSize {
		if txs.dryRun {
			txs.globalQueueExceededDryRun.Add(ctx, 1)
	...
	if q.size >= txs.maxQueueSize {
		if txs.dryRun {
			...
			txs.queueExceeded.Add(ctx, 1, tableLabel)
			return false, fmt.Errorf(
	...
	if txs.dryRun {
		txs.waitsDryRun.Add(ctx, 1, tableLabel)
	...
	txs.waits.Add(ctx, 1, tableLabel)
	select {
	case q.availableSlots <- struct{}{}:
		return true, nil
	case <-ctx.Done():
