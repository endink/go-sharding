...
// Returns a SQLError.
func (c *Conn) readColumnDefinition(field *types.Field, index int) error {
	...
	field.Type, err = types.MySQLToType(int64(t), int64(flags))
	if err != nil {
	...
	if field.ColumnLength != 0 || field.Charset != 0 {
		...
		// See doc.go.
		if IsNum(t) {
			field.Flags |= uint32(types.MySqlFlag_NUM_FLAG)
		}
	}
...
// Returns a SQLError.
func (c *Conn) readColumnDefinitionType(field *types.Field, index int) error {
	colDef, err := c.readEphemeralPacket()
	if err != nil {
		return NewSQLError(CRServerLost, SSUnknownSQLState, "%v", err)
	}
...
// Returns a SQLError.
func (c *Conn) parseRow(data []byte, fields []*types.Field) ([]types.Value, error) {
	colNumber := len(fields)
	result := make([]types.Value, colNumber)
	pos := 0
	for i := 0; i < colNumber; i++ {
		...
		result[i] = types.MakeTrusted(fields[i].Type, s)
	}
	return result, nil
}
...
func (c *Conn) ExecuteFetch(query string, maxrows int, wantfields bool) (result *types.Result, err error) {
...
func (c *Conn) ExecuteFetchMulti(query string, maxrows int, wantfields bool) (result *types.Result, more bool, err error) {
	defer func() {
		if err != nil {
...
// ExecuteFetch API.
func (c *Conn) ExecuteFetchWithWarningCount(query string, maxrows int, wantfields bool) (result *types.Result, warnings uint16, err error) {
	defer func() {
		if err != nil {
			if sqlerr, ok := err.(*SQLError); ok {
				sqlerr.Query = query
...
func (c *Conn) ReadQueryResult(maxrows int, wantfields bool) (*types.Result, bool, uint16, error) {
	// Get the result.
	...
	fields := make([]types.Field, colNumber)
	result := &types.Result{
		Fields: make([]*types.Field, colNumber),
	}
...
func (c *Conn) parseComStmtExecute(prepareData map[uint32]*PrepareData, data []byte) (uint32, byte, error) {
	...
	if ok && newParamsBoundFlag == 0x01 {
		...
		for i := uint16(0); i < prepare.ParamsCount; i++ {
			...
			valType, err := types.MySQLToType(int64(mysqlType), int64(flags))
			if err != nil {
	...
	for i := 0; i < len(prepare.ParamsType); i++ {
		...
		if (bitMap[i/8] & (1 << uint(i%8))) > 0 {
			val, pos, ok = c.parseStmtArgs(nil, types.Null, pos)
		} else {
			val, pos, ok = c.parseStmtArgs(payload, types.MySqlType(prepare.ParamsType[i]), pos)
		}
		if !ok {
			return stmtID, 0, NewSQLError(CRMalformedPacket, SSUnknownSQLState, "decoding parameter value failed: %v", prepare.ParamsType[i])
		}

		prepare.BindVars[parameterID] = types.ValueBindVariable(val)
	}

	return stmtID, cursorType, nil
}

func (c *Conn) parseStmtArgs(data []byte, typ types.MySqlType, pos int) (types.Value, int, bool) {
	switch typ {
	case types.Null:
		return types.NULL, pos, true
	case types.Int8:
		val, pos, ok := readByte(data, pos)
		return types.NewInt64(int64(int8(val))), pos, ok
	case types.Uint8:
		val, pos, ok := readByte(data, pos)
		return types.NewUint64(uint64(val)), pos, ok
	case types.Uint16:
		val, pos, ok := readUint16(data, pos)
		return types.NewUint64(uint64(val)), pos, ok
	case types.Int16, types.Year:
		val, pos, ok := readUint16(data, pos)
		return types.NewInt64(int64(int16(val))), pos, ok
	case types.Uint24, types.Uint32:
		val, pos, ok := readUint32(data, pos)
		return types.NewUint64(uint64(val)), pos, ok
	case types.Int24, types.Int32:
		val, pos, ok := readUint32(data, pos)
		return types.NewInt64(int64(int32(val))), pos, ok
	case types.Float32:
		val, pos, ok := readUint32(data, pos)
		return types.NewFloat64(float64(math.Float32frombits(uint32(val)))), pos, ok
	case types.Uint64:
		val, pos, ok := readUint64(data, pos)
		return types.NewUint64(val), pos, ok
	case types.Int64:
		val, pos, ok := readUint64(data, pos)
		return types.NewInt64(int64(val)), pos, ok
	case types.Float64:
		val, pos, ok := readUint64(data, pos)
		return types.NewFloat64(math.Float64frombits(val)), pos, ok
	case types.Timestamp, types.Date, types.Datetime:
		...
		switch size {
		case 0x00:
			return types.NewVarChar(" "), pos, ok
		case 0x0b:
			...
			return types.NewVarChar(val), pos, ok
		case 0x07:
			year, pos, ok := readUint16(data, pos)
			if !ok {
				return types.NULL, 0, false
	...
	case types.Time:
		...
		switch size {
		case 0x00:
			return types.NewVarChar("00:00:00"), pos, ok
		case 0x0c:
	...
	case types.Decimal, types.Text, types.Blob, types.VarChar, types.VarBinary, types.Char,
		types.Bit, types.Enum, types.Set, types.Geometry, types.Binary, types.Json:
		val, pos, ok := readLenEncStringAsBytesCopy(data, pos)
		return types.MakeTrusted(types.VarBinary, val), pos, ok
	default:
		return types.NULL, pos, false
	}
}
...
func (c *Conn) writeColumnDefinition(field *types.Field) error {
	...
	typ, flags := types.TypeToMySQL(field.Type)
	if field.Flags != 0 {
...
func (c *Conn) writeRow(row []types.Value) error {
	length := 0
	for _, val := range row {
		if val.IsNull() {
			length++
...
func (c *Conn) writeFields(result *types.Result) error {
...
func (c *Conn) writeRows(result *types.Result) error {
...
func (c *Conn) writePrepare(fld []*types.Field, prepare *PrepareData) error {
	...
	if paramsCount > 0 {
		for i := uint16(0); i < paramsCount; i++ {
			if err := c.writeColumnDefinition(&types.Field{
				Name:    "?",
...
func (c *Conn) writeBinaryRow(fields []*types.Field, row []types.Value) error {
	length := 0
...
func (c *Conn) writeBinaryRows(result *types.Result) error {
	for _, row := range result.Rows {
...
func val2MySQL(v types.Value) ([]byte, error) {
	var out []byte
	pos := 0
	switch v.ValueType {
	case types.Null:
	...
	case types.Timestamp, types.Date, types.Datetime:
		if len(v.Value) > 19 {
			...
			out[pos] = 0x0b
			pos++
			year, err := strconv.ParseUint(string(v.Value[0:4]), 10, 16)
			if err != nil {
				return []byte{}, err
			}
			month, err := strconv.ParseUint(string(v.Value[5:7]), 10, 8)
			if err != nil {
				return []byte{}, err
			}
			day, err := strconv.ParseUint(string(v.Value[8:10]), 10, 8)
			if err != nil {
				return []byte{}, err
			}
			hour, err := strconv.ParseUint(string(v.Value[11:13]), 10, 8)
			if err != nil {
				return []byte{}, err
			}
			minute, err := strconv.ParseUint(string(v.Value[14:16]), 10, 8)
			if err != nil {
				return []byte{}, err
			}
			second, err := strconv.ParseUint(string(v.Value[17:19]), 10, 8)
			if err != nil {
				return []byte{}, err
			}
			val := make([]byte, 6)
			count := copy(val, v.Value[20:])
			for i := 0; i < (6 - count); i++ {
				val[count+i] = 0x30
			}
			...
			second, err := strconv.ParseUint(string(v.Value[17:]), 10, 8)
			if err != nil {
				return []byte{}, err
			}
			pos = writeUint16(out, pos, uint16(year))
			...
			day, err := strconv.ParseUint(string(v.Value[8:]), 10, 8)
			if err != nil {
				return []byte{}, err
			}
			pos = writeUint16(out, pos, uint16(year))
	...
	case types.Time:
		if string(v.Value) == "00:00:00" {
			out = make([]byte, 1)
			out[pos] = 0x00
		} else if strings.Contains(string(v.Value), ".") {
			out = make([]byte, 1+12)
			out[pos] = 0x0c
			pos++

			sub1 := strings.Split(string(v.Value), ":")
			if len(sub1) != 3 {
	...
	case types.Decimal, types.Text, types.Blob, types.VarChar,
		types.VarBinary, types.Char, types.Bit, types.Enum,
		types.Set, types.Geometry, types.Binary, types.Json:
		l := len(v.Value)
		length := lenEncIntSize(uint64(l)) + l
		out = make([]byte, length)
		pos = writeLenEncInt(out, pos, uint64(l))
		copy(out[pos:], v.Value)
	default:
...
func val2MySQLLen(v types.Value) (int, error) {
	var length int
	var err error
	...
	case types.Int8, types.Uint8:
		length = 1
	case types.Uint16, types.Int16, types.Year:
		length = 2
	case types.Uint24, types.Uint32, types.Int24, types.Int32, types.Float32:
		length = 4
