...
func TestBasicPackets(t *testing.T) {
	...
	testkit.MustMatch(t, err, NewSQLError(ERAccessDeniedError, SSAccessDeniedError, "access denied: reason"), "")
	...
	err = ParseErrorPacket(data)
	testkit.MustMatch(t, err, NewSQLError(ERAccessDeniedError, SSAccessDeniedError, "access denied"), "")
...
func (t testRun) ComQuery(c *Conn, query string, callback func(*types.Result) error) error {
...
func (t testRun) ComPrepare(c *Conn, query string, bindVars map[string]*types.BindVariable) ([]*types.Field, error) {
	panic("implement me")
}

func (t testRun) ComStmtExecute(c *Conn, prepare *PrepareData, callback func(*types.Result) error) error {
	panic("implement me")
}
