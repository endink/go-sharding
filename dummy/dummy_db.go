...
var log = logging.GetLogger("fake-db-server")
...
type QueryHandler interface {
	HandleQuery(*mysql.Conn, string, func(*types.Result) error) error
}
...
func NewDB() *DB {
	...
	db := &DB{
		...
		logger:                   logging.GetLogger("fake-db"),
	}
	...
	var err error
	// Start listening.
	db.listener, err = mysql.NewListener("tcp4", db.hostAndPort, db, 0, 0, userProvider)
	if err != nil {
		panic(fmt.Errorf("NewListener failed: %v", err))
	}
...
func (db *DB) NewConnection(c *mysql.Conn) {
	...
	if conn, ok := db.connections[c.ConnectionID]; ok {
		panic(fmt.Errorf("BUG: connection with id: %v is already active. existing conn: %v new conn: %v", c.ConnectionID, conn, c))
	}
...
func (db *DB) ComQuery(c *mysql.Conn, query string, callback func(*types.Result) error) error {
...
func (db *DB) HandleQuery(c *mysql.Conn, query string, callback func(*types.Result) error) error {
	if db.AllowAll {
		return callback(&types.Result{})
	}
	...
	if db.shouldClose {
		...
		//log error
		if err := callback(&types.Result{}); err != nil {
...
func (db *DB) comQueryOrdered(query string) (*types.Result, error) {
	db.mu.Lock()
	defer db.mu.Unlock()
	...
	if index >= len(db.expectedExecuteFetch) {
		db.logger.Errorf("%v: got unexpected out of bound fetch: %v >= %v", db.name, index, len(db.expectedExecuteFetch))
	...
	if strings.HasSuffix(expected, "*") {
		if !strings.HasPrefix(query, expected[0:len(expected)-1]) {
			db.logger.Errorf("%v: got unexpected query start (index=%v): %v != %v", db.name, index, query, expected)
		}
	} else {
		if query != expected {
			db.logger.Errorf("%v: got unexpected query (index=%v): %v != %v", db.name, index, query, expected)
			return nil, errors.New("unexpected query")
		}
	}
	db.logger.Infof("ExecuteFetch: %v: %v", db.name, query)
	if entry.Error != nil {
...
func (db *DB) ComPrepare(c *mysql.Conn, query string, bindVars map[string]*types.BindVariable) ([]*types.Field, error) {
	return nil, nil
}

// ComStmtExecute is part of the mysql.Handler interface.
func (db *DB) ComStmtExecute(c *mysql.Conn, prepare *mysql.PrepareData, callback func(*types.Result) error) error {
	return nil
}
...
func (db *DB) AddQuery(query string, expectedResult *types.Result) *ExpectedResult {
...
func (db *DB) SetBeforeFunc(query string, f func()) {
	...
	r, ok := db.data[key]
	if !ok {
		db.logger.Fatalf("BUG: no query registered for: %v", query)
	}
...
func (db *DB) AddQueryPattern(queryPattern string, expectedResult *types.Result) {
	if len(expectedResult.Rows) > 0 && len(expectedResult.Fields) == 0 {
...
func (db *DB) AddQueryPatternWithCallback(queryPattern string, expectedResult *types.Result, callback func(string)) {
...
func (db *DB) VerifyAllExecutedOrFail() {
	...
	if db.expectedExecuteFetchIndex != len(db.expectedExecuteFetch) {
		db.logger.Errorf("%v: not all expected queries were executed. leftovers: %v", db.name, db.expectedExecuteFetch[db.expectedExecuteFetchIndex:])
	}
}
