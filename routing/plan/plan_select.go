...
func createSelectFieldFromByItem(p *SelectPlan, item *ast.ByItem) (*ast.SelectField, error) {
	...
	found, need, err := p.TableAliasStmtInfo.NeedCreateColumnNameExprDecoratorInField(columnExpr)
	if err != nil {
		return nil, err
	}

	if need {
		decorator := CreateColumnNameExprDecorator(columnExpr, found.Sharding, found.IsAlias, p.GetRouteResult())
		item.Expr = decorator
	}
...
func handlePatternInExpr(p *TableAliasStmtInfo, expr *ast.PatternInExpr) (bool, []int, ast.ExprNode, error) {
	rule, need, err := p.NeedCreatePatternInExprDecorator(expr)
	if err != nil {
