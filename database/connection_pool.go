...
func (cp *ConnectionPool) Open(info *mysql.ConnParams) {
	...
	defer cp.mu.Unlock()
	cp.info = info
	cp.connections = util.NewResourcePool(cp.connect, cp.capacity, cp.capacity, cp.idleTimeout, 0, nil)
...
func (cp *ConnectionPool) connect(ctx context.Context) (util.Resource, error) {
