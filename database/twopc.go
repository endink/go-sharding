...
const (
  ...
  primary key(dtid, id)
	...
	DTStatePrepare = TransactionStatePrepare
	// DTStateCommit represents the COMMIT state for dt_state.
	DTStateCommit = TransactionStateCommit
	// DTStateRollback represents the ROLLBACK state for dt_state.
	DTStateRollback       = TransactionStateRollback
...
type TwoPC struct {
	...
	insertRedoTx        *parser.ParsedQuery
	insertRedoStmt      *parser.ParsedQuery
	updateRedoTx        *parser.ParsedQuery
	deleteRedoTx        *parser.ParsedQuery
	deleteRedoStmt      *parser.ParsedQuery
	readAllRedo         string
	countUnresolvedRedo *parser.ParsedQuery

	insertTransaction   *parser.ParsedQuery
	insertParticipants  *parser.ParsedQuery
	transition          *parser.ParsedQuery
	deleteTransaction   *parser.ParsedQuery
	deleteParticipants  *parser.ParsedQuery
	readTransaction     *parser.ParsedQuery
	readParticipants    *parser.ParsedQuery
	readAbandoned       *parser.ParsedQuery
...
func (tpc *TwoPC) SaveRedo(ctx context.Context, conn *StatefulConnection, dtid string, queries []string) error {
	bindVars := map[string]*types.BindVariable{
		"dtid":         types.StringBindVariable(dtid),
		"state":        types.Int64BindVariable(RedoStatePrepared),
		"time_created": types.Int64BindVariable(time.Now().UnixNano()),
	}
	...
	rows := make([][]types.Value, len(queries))
	for i, query := range queries {
		rows[i] = []types.Value{
			types.NewVarBinary(dtid),
			types.NewInt64(int64(i + 1)),
			types.NewVarBinary(query),
		}
	}
	extras := map[string]parser.Encodable{
		"vals": parser.InsertValues(rows),
	}
...
func (tpc *TwoPC) UpdateRedo(ctx context.Context, conn *StatefulConnection, dtid string, state int) error {
	bindVars := map[string]*types.BindVariable{
		"dtid":  types.StringBindVariable(dtid),
		"state": types.Int64BindVariable(int64(state)),
	}
...
func (tpc *TwoPC) ReadAllRedo(ctx context.Context) (prepared, failed []*PreparedTx, err error) {
	...
	for _, row := range qr.Rows {
		...
		if curTx == nil || dtid != curTx.Dtid {
			...
			// which is harmless.
			tm, _ := types.ToInt64(row[2])
			curTx = &PreparedTx{
				Dtid: dtid,
				Time: time.Unix(0, tm),
			}
			st, err := types.ToInt64(row[1])
			if err != nil {
...
func (tpc *TwoPC) CountUnresolvedRedo(ctx context.Context, unresolvedTime time.Time) (int64, error) {
	...
	bindVars := map[string]*types.BindVariable{
		"time_created": types.Int64BindVariable(unresolvedTime.UnixNano()),
	}
	...
	v, _ := types.ToInt64(qr.Rows[0][0])
	return v, nil
}

// CreateTransaction saves the metadata of a 2pc transaction as Prepared.
func (tpc *TwoPC) CreateTransaction(ctx context.Context, conn *StatefulConnection, dtid string, participants []*Target) error {
	bindVars := map[string]*types.BindVariable{
		"dtid":     types.StringBindVariable(dtid),
		"state":    types.Int64BindVariable(int64(DTStatePrepare)),
		"cur_time": types.Int64BindVariable(time.Now().UnixNano()),
	}
	...
	rows := make([][]types.Value, len(participants))
	for i, participant := range participants {
		rows[i] = []types.Value{
			types.NewVarBinary(dtid),
			types.NewInt64(int64(i + 1)),
			types.NewVarBinary(participant.Schema),
			types.NewVarBinary(participant.DataSource),
		}
	}
	extras := map[string]parser.Encodable{
		"vals": parser.InsertValues(rows),
...
func (tpc *TwoPC) Transition(ctx context.Context, conn *StatefulConnection, dtid string, state TransactionState) error {
	bindVars := map[string]*types.BindVariable{
		"dtid":    types.StringBindVariable(dtid),
		"state":   types.Int64BindVariable(int64(state)),
		"prepare": types.Int64BindVariable(int64(TransactionStatePrepare)),
	}
	...
	if qr.RowsAffected != 1 {
		return fmt.Errorf("could not transition to %v: %s", state, dtid)
	}
	return nil
}
...
func (tpc *TwoPC) ReadTransaction(ctx context.Context, dtid string) (*TransactionMetadata, error) {
	conn, err := tpc.readPool.Get(ctx)
	if err != nil {
		return nil, err
	}
	...
	st, err := types.ToInt64(qr.Rows[0][1])
	if err != nil {
		return nil, util.Wrapf(err, "Error parsing state for dtid %s", dtid)
	}
	result.State = TransactionState(st)
	if result.State < TransactionStatePrepare || result.State > TransactionStateRollback {
		return nil, fmt.Errorf("unexpected state for dtid %s: %v", dtid, result.State)
	}
	// A failure in time parsing will show up as a very old time,
	// which is harmless.
	tm, _ := types.ToInt64(qr.Rows[0][2])
	...
	participants := make([]*Target, 0, len(qr.Rows))
	for _, row := range qr.Rows {
		participants = append(participants, &Target{
			Schema:     row[0].ToString(),
			DataSource: row[1].ToString(),
...
func (tpc *TwoPC) ReadAbandoned(ctx context.Context, abandonTime time.Time) (map[string]time.Time, error) {
	...
	bindVars := map[string]*types.BindVariable{
		"time_created": types.Int64BindVariable(abandonTime.UnixNano()),
	}
...
func (tpc *TwoPC) ReadAllTransactions(ctx context.Context) ([]*DistributedTx, error) {
	...
	for _, row := range qr.Rows {
		...
		if curTx == nil || dtid != curTx.Dtid {
			...
			protostate := TransactionState(st)
			if protostate < TransactionStateUnknown || protostate > TransactionStateRollback {
				log.Errorf("Unexpected state for dtid %s: %v.", dtid, protostate)
			}
			curTx = &DistributedTx{
				Dtid:    dtid,
				State:   TransactionState(st).String(),
				Created: time.Unix(0, tm),
			}
			distributed = append(distributed, curTx)
		}
		curTx.Participants = append(curTx.Participants, Target{
			Schema:     row[3].ToString(),
			DataSource: row[4].ToString(),
		})
	}
	return distributed, nil
}

func (tpc *TwoPC) exec(ctx context.Context, conn *StatefulConnection, pq *parser.ParsedQuery, bindVars map[string]*types.BindVariable) (*types.Result, error) {
...
func (tpc *TwoPC) read(ctx context.Context, conn *DBConn, pq *parser.ParsedQuery, bindVars map[string]*types.BindVariable) (*types.Result, error) {
	q, err := pq.GenerateQuery(bindVars, nil)
	if err != nil {
		return nil, err
	}
