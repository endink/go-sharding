...
func TestTxEngineClose(t *testing.T) {
	...
	te.AcceptReadWrite()
	c, beginSQL, err := te.txPool.Begin(ctx, &types.ExecuteOptions{}, false, 0, nil)
	...
	// Immediate close.
	te.AcceptReadOnly()
	c, _, err = te.txPool.Begin(ctx, &types.ExecuteOptions{}, false, 0, nil)
	if err != nil {
		t.Fatal(err)
	}
	c.Unlock()
	...
	go func() {
		...
		te.txPool.CompleteAndRelease(ctx, c)
	}()
	start = time.Now()
	te.Close()
	...
	te.AcceptReadWrite()
	te.AcceptReadWrite()
	_, err = te.Reserve(ctx, &types.ExecuteOptions{}, 0, nil)
	require.NoError(t, err)
	_, err = te.ReserveBegin(ctx, &types.ExecuteOptions{}, nil)
	require.NoError(t, err)
	start = time.Now()
	te.Close()
	assert.Less(t, int64(50*time.Millisecond), int64(time.Since(start)))
...
func startTransaction(te *TxEngine, writeTransaction bool) error {
	options := &types.ExecuteOptions{}
	if writeTransaction {
		options.TransactionIsolation = types.IsolationDefault
	} else {
